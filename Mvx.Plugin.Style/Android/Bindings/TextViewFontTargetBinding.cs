...
namespace Mvx.Plugin.Style.Droid.Bindings
{
    public class TextViewFontTargetBinding
		...
		public TextViewFontTargetBinding(TextView target): base(target) 
        {
			if (target == null) 
            {
                MvxBindingLog.Instance.Log(LogLevel.Error, "Error - textview is null in TextViewFontTargetBinding");
				return;
			}
		}
		...
		protected override void SetValueImpl(object target, object toSet) {
			...
			if (font != null)
            {
                ...
                {
                    MvxBindingLog.Instance.Log(LogLevel.Error, "Failed to set font to Textview. Check if font exists, has a size and filename");
				}
			}
		}
