...
namespace Redhotminute.Plugin.Style
{
	public class FontResourceValueConverter : MvxValueConverter {
		...
		private object ConvertValue(object value,object parameter) {
			...
			if (plugin == null) {
				plugin = (Redhotminute.Plugin.Style.AssetPlugin)Mvx.Resolve<IAssetPlugin>();
			}
			...
			if (value!= null && parameter!= null){
				try{
					...
					Mvx.Trace(MvvmCross.Platform.Platform.MvxTraceLevel.Error, $"Failed to get font {e.Message}");
				}
			}
